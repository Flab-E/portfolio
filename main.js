  ...
  async function loadFile(filename, pushHistory = true) {
    ...
    try {
      ...
      if (!resp || !resp.ok) {
        ...
        ({ resp, candidate } = await tryFetchVariants(
          baseDir,
          "./" + filename,
        ));
      }
      ...
      if (!resp || !resp.ok) {
        viewer.innerHTML = `<div class="blog-content"><p class="text-red-400">Failed to load ${filename}</p></div>`;
        return;
      }
      ...
      if (!resp.ok) {
        viewer.innerHTML = `<div class="blog-content"><p class="text-red-400">Failed to load ${filename}: ${resp.status} ${resp.statusText}</p></div>`;
        return;
      }
      ...
      if (ext === "md" || ext === "markdown") {
        ...
        const html =
          typeof marked !== "undefined"
            ...
            : `<pre>${escapeHtml(preprocessed)}</pre>`;
        viewer.innerHTML = `<div class="blog-content">${html}</div>`;
        ...
        viewer.innerHTML = `<div class="blog-content"><pre>${escapeHtml(preprocessed)}</pre></div>`;
      } else if (ext === "html") {
      ...
      viewer.innerHTML = `<div class="blog-content"><p class="text-red-400">Error loading ${filename}: ${escapeHtml(err.message)}</p></div>`;
    }
  }
    ...
    mobileMenuBtn.addEventListener("click", () => {
      ...
      if (!mobileMenu) {
        ...
        mobileMenu.className =
          "md:hidden bg-primary-dark border-t border-grey-800 py-4";
  ...
  timelineItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      timelineItems.forEach((otherItem) => {
        ...
        const otherHeight =
          otherContent.scrollHeight || otherContent.offsetHeight || 0;
...
function initializeBackToTop() {
  ...
  backToTopBtn.className =
    "fixed bottom-6 right-6 w-12 h-12 bg-accent-teal text-white rounded-full shadow-lg opacity-0 transition-all duration-300 z-40";
